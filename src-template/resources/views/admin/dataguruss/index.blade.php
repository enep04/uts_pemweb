...
@can('dataguru_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.dataguruss.create') }}">
                ...
                <tbody>
                    @foreach($datagurus as $key => $d)
                        <tr data-entry-id="{{ $d->id }}">
                            ...
                            <td>
                                {{ App\Models\Dataguru::RIWAYAT_PENDIDIKAN[$d->riwayat_pendidikan] ?? '' }}
                            </td>
                            <td>
                                {{ $d->universitas ?? '' }}
                            </td>
                            ...
                            <td>
                                @can('dataguru_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.dataguruss.show', $d->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('dataguru_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.dataguruss.edit', $d->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('dataguru_delete')
                                    <form action="{{ route('admin.dataguruss.destroy', $d->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        @method('DELETE')
                                        @csrf
...
@can('dataguru_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.dataguruss.massDestroy') }}",
